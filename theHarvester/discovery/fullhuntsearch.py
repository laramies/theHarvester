...
class SearchFullHunt:
    ...
    GENERAL_FILTERS = [
        'domain', 'ip', 'tech', 'host', 'subdomain', 'tld', 'tag', 'is_dos_defense'
    ]
    
    NETWORK_FILTERS = [
        'port', 'has_private_ip', 'has_ipv6', 'is_live', 'is_resolvable',
        'dns_a', 'dns_aaaa', 'dns_cname', 'dns_mx', 'dns_txt', 'dns_ptr', 'dns_ns'
    ]
    
    HTTP_FILTERS = [
        'http_title', 'http_status_code', 'http_favicon_hash'
    ]
    
    GEO_FILTERS = [
        'country_code', 'country', 'city', 'asn'
    ]
    
    CLOUD_FILTERS = [
        'is_cloud', 'cloud_provider', 'cloud_region'
    ]
    ...
    CERT_FILTERS = [
        'cert_issuer_common_name', 'cert_issuer_organization', 'cert_issuer_country',
        'cert_issuer_serial_number', 'cert_signature_algorithm', 'cert_subject_common_name',
        'cert_subject_country', 'cert_subject_province', 'cert_subject_locality',
        'cert_subject_organization', 'cert_md5_fingerprint', 'cert_sha1_fingerprint'
    ]
    
    # Combine all filters for validation
    ALL_FILTERS = (
        GENERAL_FILTERS + NETWORK_FILTERS + HTTP_FILTERS + 
        GEO_FILTERS + CLOUD_FILTERS + TECH_FILTERS + CERT_FILTERS
    )
    ...
    async def extract_data_from_domain_details(self, details: Dict[str, Any]) -> None:
        ...
        for host_data in hosts:
            # Extract subdomains
            if 'host' in host_data and host_data['host']:
            ...
            # Extract ports
            if 'network_ports' in host_data and host_data['network_ports']:
                self.total_results['ports'].update(host_data['network_ports'])
                
            # Extract technologies/products
            if 'products' in host_data and host_data['products']:
                self.total_results['technologies'].extend(host_data['products'])
                
            # Extract tags
            if 'tags' in host_data and host_data['tags']:
    ...
    async def process(self, proxy: bool = False, filters: Optional[Dict[str, str]] = None) -> None:
