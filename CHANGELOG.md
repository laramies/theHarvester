...
### Changed
- Replaced Shodan's synchronous Python SDK with cancellable async Host API requests that honor configured proxies, query every unique resolved IPv4 sequentially, retain successful partial results and existing evidence formats, and add no source-local deadline.
