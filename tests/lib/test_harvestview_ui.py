...
def test_harvestview_serves_pinned_tabulator_locally(tmp_path, monkeypatch) -> None:
    from theHarvester.lib.api import api
    ...
    assert '<link rel="stylesheet" href="/static/harvestview/tabulator.min.css">' in schedules.text
    assert '<script src="/static/harvestview/tabulator.min.js"></script>' in root.text
    assert '<script src="/static/harvestview/tabulator.min.js"></script>' in schedules.text
    assert 'cdnjs.cloudflare.com' not in root.text
    assert 'cdnjs.cloudflare.com' not in schedules.text
    assert theme.status_code == 200
    assert script.status_code == 200
    assert license_file.status_code == 200
    assert 'Tabulator' in script.text
