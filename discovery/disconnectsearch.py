...
# https://search.disconnect.me/searchTerms/search?query="microsoft.com"&ses=Google
...
class search_disconnect:
    ...
    def do_search_files(self, files):
        r = requests.get(
            "https://" + self.hostname + "/searchTerms/search?query=filetype:" + files + "%20site:" + self.word + "&ses=" + self.engine)
        returncode = r.status_code
        headers = r.headers
        self.results = r.text
    ...
    def do_search_profiles(self):
        r = requests.get(
            "https://" + self.hostname + '/searchTerms/search?query=site:www.google.com%20intitle:"Google%20Profile"%20"Companies%20I%27ve%20worked%20for"%20"at%20' + self.word + "&ses=" + self.engine)
        returncode = r.status_code
        headers = r.headers
        self.results = r.text
        self.totalresults += self.results
    ...
    def process(self):
        ...
        while self.counter <= self.limit and self.counter <= 1000:
            self.do_search()
			########################################################
			# Follow links to next pages and add them to results
