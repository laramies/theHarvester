...
- `--routeviews` enriches an explicitly targeted ASN, IP, or CIDR with bounded observed-origin, BGP route, and RPKI evidence. For example, use `-d AS16509 --routeviews` for an intentional ASN pivot. ASNs merely discovered during a domain run are retained but not expanded into a provider's global prefix table. RouteViews is a P0 action, is not selected by `-b all`, and does not use `-l`. A configured `routeviews.key` is used automatically for PeeringDB-verified authenticated access; otherwise the action uses the guest allowance.
