...
- `--routeviews` enriches ASNs returned by selected sources, or an explicitly targeted ASN, IP, or CIDR, with bounded observed-origin, BGP route, and RPKI evidence. For example, `-d example.com -b asns --routeviews` enriches discovered ASNs, while `-d AS16509 --routeviews` performs an intentional ASN pivot. Shared cloud and CDN prefixes remain external relationships rather than claimed engagement scope. RouteViews is a P0 action, is not selected by `-b all`, and does not use `-l`. A configured `routeviews.key` is used automatically for PeeringDB-verified authenticated access; otherwise the action uses the guest allowance.
