...
| `GET /api/v1/runs/{run_id}` | Retrieve lifecycle state, options, results, source contributions, hostname comparisons, and artifacts. |
...
curl -s "http://127.0.0.1:5000/api/v1/runs/$run_id" \
  -H "X-API-Key: $THEHARVESTER_API_KEY" \
  | jq '{status, evidence_status, results, source_executions, source_contributions, hostname_comparison, action_executions, artifacts}'
```
...
Finalized run details include a `hostname_comparison` object derived only from persisted SQLite evidence. Reading the endpoint never starts discovery or DNS. The selected run is compared with the latest earlier finalized run that has the same canonical target and exact source list.
