...
| Serply | Set `serply.key` under `apikeys` in `api-keys.yaml`, then run `uv run theHarvester -d example.com -b serply -l 100`. Serply reads Google's index. Each request costs one API credit and returns at most ten organic rows, so `-l` bounds both results and spend. Pagination uses the `start` offset, stops once a page repeats the previous window, and is bounded at ten pages per query. See [Serply](https://serply.io/docs) for current plans and quotas. |
