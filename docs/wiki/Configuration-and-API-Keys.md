...
| JSMON | Set `jsmon.key` under `apikeys`, or set `JSMON_KEY` in the process environment to override it. Load a local `.env` with `uv run --env-file .env theHarvester -d example.com -b jsmon -l 0`. Pagination follows the API's reported pages; `-l 0` removes the local result cap. HTTP 403 or 429 stops collection without retrying and preserves earlier results. See [JSMON](https://subdomains.jsmon.sh/login) for current access limits. |
